...
*   **Production Build Command:** Always run `npm run dist` to package the production-ready Electron app.
*   **Release Assets Naming:** Assets **MUST** use hyphens (e.g., `CmdGUI-Setup-1.5.0.exe`) as configured in `package.json` to ensure auto-updater compatibility.
*   **Required GitHub Release Assets:** Every GitHub release **MUST** contain these three files:
    1.  The `.exe` installer (with hyphenated name).
    2.  The `.exe.blockmap` file (generated by electron-builder).
