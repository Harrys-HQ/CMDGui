...
fn get_settings_path() -> PathBuf {
    let base_dir = if let Ok(appdata) = std::env::var("APPDATA") {
        PathBuf::from(appdata).join("cmd-gui")
    } else if let Ok(home) = std::env::var("HOME") {
        PathBuf::from(home).join(".config").join("cmd-gui")
    } else {
        PathBuf::from(".")
    };
...
pub async fn settings_get(key: Option<String>) -> Result<serde_json::Value, String> {
    ...
    let settings: serde_json::Value = serde_json::from_str(&content).unwrap_or(serde_json::Value::Object(serde_json::Map::new()));
